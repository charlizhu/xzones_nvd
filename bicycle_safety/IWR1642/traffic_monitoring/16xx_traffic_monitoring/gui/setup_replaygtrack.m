function varargout = setup_replaygtrack(varargin)
% SETUP_REPLAYGTRACK MATLAB code for setup_replaygtrack.fig
%      SETUP_REPLAYGTRACK, by itself, creates a new SETUP_REPLAYGTRACK or raises the existing
%      singleton*.
%
%      H = SETUP_REPLAYGTRACK returns the handle to a new SETUP_REPLAYGTRACK or the handle to
%      the existing singleton*.
%
%      SETUP_REPLAYGTRACK('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SETUP_REPLAYGTRACK.M with the given input arguments.
%
%      SETUP_REPLAYGTRACK('Property','Value',...) creates a new SETUP_REPLAYGTRACK or raises
%      the existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before setup_replaygtrack_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to setup_replaygtrack_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help setup_replaygtrack

% Last Modified by GUIDE v2.5 31-May-2018 09:02:53

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @setup_replaygtrack_OpeningFcn, ...
                   'gui_OutputFcn',  @setup_replaygtrack_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before setup_replaygtrack is made visible.
function setup_replaygtrack_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to setup_replaygtrack (see VARARGIN)

% Choose default command line output for setup_replaygtrack
...
function varargout = setup_replaygtrack_OutputFcn(hObject, eventdata, handles)
...
varargout{1} = handles;
varargout{2} = handles.fhistPath;
varargout{3} = handles.fhistFiles;
varargout{4} = handles.mode;
...
%     end
    ...
    setup_replaygtrack_OutputFcn(hObject,eventdata,guidata(hObject));
    uiresume(gcbf);
else
    warndlg('Error: No files selected!');
end
...
if (filename ~= 0)
    ...
    if(isfield(Params, 'gtrackAdvanced'))
        ...
        if(isfield(Params.gtrackAdvanced,'gatingParam'))
            displayGtrackParams(Params.gtrackAdvanced.gatingParam, findobj('Tag', 'uitableGating'));
        end
        
        if(isfield(Params.gtrackAdvanced,'variationParam'))
            displayGtrackParams(Params.gtrackAdvanced.variationParam, findobj('Tag', 'uitableVariation'));
        end
...
% --- Executes on button press in pushbuttonSelectFiles.
...
handles.fhistPath = path;
handles.fhistFiles = files;
hTextSavePath = findobj('Tag', 'textSavePath');
if(~iscell(files))
    if(files == 0)
        hTextSavePath.String = 'NO FILES SELECTED';
    else
        hTextSavePath.String = ['Loaded file: ' files];
    end
else
    hTextSavePath.String = ['Loaded files: ' files{1} ' ... ' files{end}];   
end
...
% --- Executes on button press in radiobuttonTarget.
function radiobuttonTarget_Callback(hObject, eventdata, handles)
% hObject    handle to radiobuttonTarget (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobuttonTarget


% --- Executes on button press in radiobuttonPC.
function radiobuttonPC_Callback(hObject, eventdata, handles)
% hObject    handle to radiobuttonPC (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobuttonPC


% --- Executes on button press in checkboxEnableVideo.
function checkboxEnableVideo_Callback(hObject, eventdata, handles)
% hObject    handle to checkboxEnableVideo (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkboxEnableVideo


% --- Executes when selected object is changed in uibuttongroupMode.
function uibuttongroupMode_SelectionChangedFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in uibuttongroupMode 
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
modeTag = get(get(handles.uibuttongroupMode,'SelectedObject'),'Tag');
switch modeTag
    case 'radiobuttonTarget'
        handles.mode = 1;
        set(handles.uipanelGtrack, 'Visible', 'off');
        
    case 'radiobuttonPC'
        handles.mode = 2;
        set(handles.uipanelGtrack, 'Visible', 'on');        
end
guidata(hObject, handles)
